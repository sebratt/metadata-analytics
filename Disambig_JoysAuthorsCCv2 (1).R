...
Author_tracking <- read_xlsx("Author_tracking_QLiu.xlsx") # read in author data - ambiguous
...
# semantic scholar data from Jun ^^
...
# example 1:  Fujiwara T. (a name in Joy's list that has >10 papers per year i.e. 17)
...
# Check Result 3
result4 <- result3[,c(1,7,2,4,5,6,8,9,10)]

# example 2:  Cao,J.. (a name in Joy's list that has >5 papers per year)

# example 3:  Tamura,T. (a name in Joy's list that has >5 papers per year)

# example 4:  Wang,K.. (a name in Joy's list that has >5 papers per year)

# example 5: Cui 


result4$iz.na <- is.na(result4[,2])
table(result4$iz.na)
result5<-result4[!result4$iz.na,]
write.csv(result5, "disambig_verify.csv")
